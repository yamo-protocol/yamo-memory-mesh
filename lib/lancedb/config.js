...
function loadConfig() {
  ...
  // Note: This resolves at config load time; for lazy resolution, use relative paths in client
  if (config.LANCEDB_URI.startsWith('./') || config.LANCEDB_URI.startsWith('../')) {
    try {
      ...
      // Keep relative if cwd unavailable (e.g., during npm publish)
      // Client will resolve at connect time
    }
  }
