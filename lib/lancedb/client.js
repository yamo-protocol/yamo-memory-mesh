...
class LanceDBClient {
   ...
   */
  constructor(config = {}) {
    this.uri = config.uri || process.env.LANCEDB_URI || './data/lancedb';
    this.tableName = config.tableName || process.env.LANCEDB_MEMORY_TABLE || 'memory_entries';
    this.maxRetries = config.maxRetries || 3;
    this.retryDelay = config.retryDelay || 1000;
    this.vectorDimension = config.vectorDimension || DEFAULT_VECTOR_DIMENSION;
  ...
  async connect() {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        // Resolve relative URI to absolute path at connect time (not during config load)
        // This avoids process.cwd() issues during package publishing
        const uri = this.uri.startsWith('./') || this.uri.startsWith('../')
          ? path.resolve(process.cwd(), this.uri)
          : this.uri;

        // Ensure database directory exists
        const dbPath = path.resolve(uri);
    ...
    throw new StorageError(
      `Failed to connect to LanceDB after ${this.maxRetries} attempts: ${lastError.message}`,
  ...
  async getStats() {
    ...
    return await this._retryOperation(async () => {
      ...
      try {
        ...
        // If all counting methods fail, mark as unknown
        count = 'unknown';
      }
   ...
   */
  async _retryOperation(operation, maxRetries = null, baseDelay = null) {
    ...
    for (let attempt = 1; attempt <= max; attempt++) {
      try {
        ...
        console.warn(
          `[LanceDBClient] Retryable error on attempt ${attempt}/${max}: ${error.message}. ` +
