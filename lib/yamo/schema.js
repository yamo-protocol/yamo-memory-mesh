...
export function createYamoSchema() {
    return new arrow.Schema([
        ...
        new arrow.Field("anchored_at", new arrow.Timestamp(arrow.TimeUnit.MILLISECOND), true), // Timestamp when anchored
...
export async function createYamoTable(db, tableName = "yamo_blocks") {
    try {
        ...
        if (existingTables.includes(tableName)) {
            ...
            const hasVectorField = existingSchema.fields.some((f) => f.name === "vector" && f.type.constructor.name === "FixedSizeList");
            if (hasVectorField) {
                ...
                if (!fieldNames.includes("prev_hash")) {
                    newCols.push({ name: "prev_hash", valueSql: "cast(null as string)" });
                }
                if (!fieldNames.includes("anchored_at")) {
                    newCols.push({ name: "anchored_at", valueSql: "cast(null as timestamp)" });
                }
                if (newCols.length > 0) {
                    try {
