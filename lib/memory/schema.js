 ...
 */
export const MEMORY_STATES = ["active", "superseded", "deprecated", "archived"];
/**
 ...
 */
export const DECISION_RELATIONS = ["supersedes", "depends-on", "justified-by", "contradicts"];
/**
 ...
 */
export function createDecisionEdgeSchema() {
    return new arrow.Schema([
        new arrow.Field("id", new arrow.Utf8(), false),
        new arrow.Field("source_id", new arrow.Utf8(), false),
        new arrow.Field("target_id", new arrow.Utf8(), false),
        new arrow.Field("relation", new arrow.Utf8(), false),
        new arrow.Field("rationale", new arrow.Utf8(), true),
        new arrow.Field("weight", new arrow.Float32(), false),
        new arrow.Field("created_at", new arrow.Timestamp(arrow.TimeUnit.MILLISECOND), false),
    ]);
}
 ...
 */
export async function createDecisionEdgeTable(db, tableName = "decision_edges") {
    try {
        const existingTables = await db.tableNames();
        if (existingTables.includes(tableName)) {
            return await db.openTable(tableName);
            ...
            const schema = createDecisionEdgeSchema();
            return await db.createTable(tableName, [], {
                schema,
                storageOptions: { new_table_data_storage_version: "stable" },
            });
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new Error(`Failed to create decision edge table ${tableName}: ${message}`);
    }
}
/**
 ...
 */
export async function createRevisionTable(db, tableName = "memory_revisions") {
    try {
        const existingTables = await db.tableNames();
        if (existingTables.includes(tableName)) {
            return await db.openTable(tableName);
