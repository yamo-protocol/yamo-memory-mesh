...
class EmbeddingFactory {
    ...
    async embedTokens(text, options = {}) {
        ...
        if (typeof this.primaryService.embedTokens !== 'function') return null;
        if (!this.primaryService.initialized) await this.primaryService.init();
        try {
    ...
    async colbertRerank(queryText, candidates, options = {}) {
        ...
        if (!queryTokens) return candidates; // Bail: model unsupported
    ...
    async rerank(query, documents) {
        ...
        const scores: number[] = new Array(documents.length);
