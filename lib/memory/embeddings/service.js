 ...
 */
export function getInstructionPrefix(modelName) {
    if (!modelName)
        return null;
    if (process.env.EMBEDDING_INSTRUCTION_PREFIXES === 'off')
        return null;
...
export class EmbeddingService {
    ...
    async embedLateChunked(fullText, spans, options = {}) {
        ...
        if (this.modelType !== 'local')
            return null; // Token-level path is HF/ONNX-only.
        ...
        try {
            ...
            if (!tokenEmbeddings || tokenEmbeddings.length !== numTokens * dim)
                return null;
            ...
            for (const span of spans) {
                ...
                for (let t = 0; t < numTokens; t++) {
                    ...
                    if (tStart === 0 && tEnd === 0)
                        continue; // [CLS]/[SEP]/[PAD] special tokens
    ...
    async embedTokens(text, options = {}) {
        ...
        try {
            ...
            if (!raw || raw.length !== numTokens * dim)
                return null;
            ...
            if (tokenizer && (typeof tokenizer === 'function' || typeof tokenizer.encode === 'function')) {
                try {
                    const encoded = await (typeof tokenizer === 'function'
                        ? tokenizer(prefixedText, { return_offsets_mapping: true })
                        : tokenizer.encode(prefixedText, { return_offsets_mapping: true }));
                    let offsets = encoded?.offset_mapping ?? encoded?.offsets;
                    if (offsets) {
                        if (offsets.dims && Array.isArray(offsets.data)) {
                            const flat = offsets.data;
                            const pairs = [];
                        ...
                        if (Array.isArray(offsets) && Array.isArray(offsets[0])) {
                            keepMask = offsets.map(([s, e]) => !(s === 0 && e === 0));
                        }
                    }
                }
                catch {
            ...
            for (let i = 0; i < kept.length; i++) {
                ...
                if (this.normalize) {
                    ...
                    if (mag > 0) {
                        for (let d = 0; d < dim; d++)
                            packed[dstBase + d] = packed[dstBase + d] / mag;
                    }
                }
            }
