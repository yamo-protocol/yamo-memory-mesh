 ...
 */
export const DECISION_RELATIONS = ["supersedes", "depends-on", "justified-by", "contradicts"] as const;
 ...
 */
export async function createDecisionEdgeTable(db: any, tableName = "decision_edges") {
    try {
        const existingTables = await db.tableNames();
        if (existingTables.includes(tableName)) {
            return await db.openTable(tableName);
