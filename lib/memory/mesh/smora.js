 ...
 */
export async function smora(mesh, query, options = {}) {
    await mesh.init();
    const t0 = Date.now();
    ...
    const fusedEntries = rrfMerge([
        { items: semanticOrig, weight: 1.0 },
        { items: semanticHyde, weight: 0.8 },
        {
            items: keywordResults.map((r) => ({ id: r.id, content: r.content, metadata: r.metadata, created_at: r.created_at || new Date().toISOString() })),
            weight: 0.6,
        },
    ]);
    ...
    if (enableSynthesis && mesh.llmClient) {
        try {
            ...
            synthesis = await mesh.llmClient.complete(`You are a retrieval synthesis agent. Given the following memory excerpts, produce a coherent summary that directly answers the query.\nQuery: ${scrubbed}\nExcerpts:\n${excerpts}`);
            synthesized = true;
        }
 ...
 * @private
 */
export function _canonicalizeIntent(_mesh, intent) {
 ...
 * @private
 */
export async function _embedIntent(mesh, intent) {
    const key = mesh._canonicalizeIntent(intent);
    if (!key)
        return null;
    if (mesh.intentEmbedCache.has(key))
        return mesh.intentEmbedCache.get(key);
    try {
        ...
        if (mesh.intentEmbedCache.size >= 500) {
            ...
            if (firstKey !== undefined)
                mesh.intentEmbedCache.delete(firstKey);
        }
 ...
 * @private
 */
export function _heritageBonusFromVectors(_mesh, sessionVecs, chainVecs, denom) {
 ...
 */
export async function _generateHyDE(mesh, query) {
    ...
    try {
        ...
        if (mesh.hydeCache.size >= cap) {
            ...
            if (firstKey !== undefined)
                mesh.hydeCache.delete(firstKey);
        }
