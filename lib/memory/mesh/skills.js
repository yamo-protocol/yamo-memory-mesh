 ...
 */
export async function ingestSkill(mesh, yamoText, metadata = {}, sourceFilePath, opts = {}) {
    await mesh.init();
    if (!mesh.skillTable) {
 ...
 */
export async function commitPendingIngest(mesh, pending, opts = {}) {
    await mesh.init();
    if (!mesh.skillTable) {
        throw new Error("Skill table not initialized");
    }
 ...
 */
export async function _acquireStagingLock(mesh) {
    let release;
 ...
 */
export async function synthesize(mesh, options = {}) {
    await mesh.init();
 ...
 * Update reliability
 */
export async function updateSkillReliability(mesh, id, success) {
    await mesh.init();
    if (!mesh.skillTable) {
        throw new Error("Skill table not initialized");
    }
 ...
 */
export async function getSkill(mesh, id) {
    await mesh.init();
    if (!mesh.skillTable) {
        return null;
    }
 ...
 * Prune skills
 */
export async function pruneSkills(mesh, threshold = 0.3) {
    await mesh.init();
    if (!mesh.skillTable) {
        throw new Error("Skill table not initialized");
    }
 ...
 */
export async function listSkills(mesh, options = {}) {
    await mesh.init();
    if (!mesh.skillTable) {
        return [];
    }
 ...
 * @returns {Promise<Array>} Normalized skill results
 */
export async function searchSkills(mesh, query, options = {}) {
    await mesh.init();
    if (!mesh.skillTable) {
        return [];
    }
    try {
        ...
        const fused = rrfMerge([
            { items: vectorResults, weight: 0.4 },
