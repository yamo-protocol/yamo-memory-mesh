 ...
 */
export async function reflect(mesh: MemoryMesh, options: { lookback?: number; topic?: string; generate?: boolean } = {}) {
    await mesh.init();
 ...
 */
export async function raptor(mesh: MemoryMesh, options: {
    topic?: string;
    limit?: number;
    maxLevels?: number;
 ...
 * @private
 */
export async function _summarizeCluster(mesh: MemoryMesh,
    ...
    if (!mesh.enableLLM || !mesh.llmClient) return null;
