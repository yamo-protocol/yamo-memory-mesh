...
export async function _applyGraphRagBoosting(mesh, results, query) {
 ...
 * @private
 */
export function _canonicalizeEntity(_mesh, entity) {
 ...
 * @private
 */
export function _contentMentions(mesh, content, entity) {
    if (!entity || !content)
        return false;
 ...
 */
export function _extractTriplesHeuristics(mesh, content) {
...
export async function _extractTriplesLLM(mesh, content) {
    if (!mesh.llmClient)
        return [];
    try {
