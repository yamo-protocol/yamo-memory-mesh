 ...
 */
export async function getYamoLog(mesh: MemoryMesh, options: { limit?: number } = {}) {
    if (!mesh.yamoTable) {
        return [];
    }
 ...
 * @private
 */
export async function _quarantineYamoTable(mesh: MemoryMesh, cause: any): Promise<void> {
    if (!mesh.dbDir || mesh.dbDir === ":memory:") return;
 ...
 */
export async function _emitYamoBlock(mesh: MemoryMesh, operationType: string, memoryId: string | undefined, yamoText: string, heritage?: { intentChain: string[]; hypotheses: string[]; rationales: string[] }) {
    if (!mesh.yamoTable) {
        return;
    }
...
export async function anchor(mesh: MemoryMesh) {
    await mesh.init();
    if (!mesh.yamoTable) {
