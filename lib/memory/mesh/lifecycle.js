...
import { MEMORY_STATES } from "../schema.js";
 ...
 */
export function _activeStateClause(_mesh, opts = {}) {
 ...
 */
export function _coerceDeferUntil(_mesh, value) {
 ...
 */
export async function setState(mesh, id, state) {
    await mesh.init();
 ...
 */
export async function deferMemory(mesh, id, until) {
    await mesh.init();
    if (!mesh.client) {
        throw new Error("Database client not initialized");
    }
 ...
 */
export function _recordRevision(mesh, memoryId, changes, actor) {
    if (!mesh.revisionTable || changes.length === 0)
        return;
    ...
    mesh.revisionTable.add(rows).catch((e) => {
 ...
 * old/new values.
 */
export async function history(mesh, memoryId) {
    await mesh.init();
 ...
 */
export async function restoreDeleted(mesh, id) {
    await mesh.init();
    if (!mesh.client) {
        throw new Error("Database client not initialized");
    }
 ...
 */
export async function _resolveIdOrKey(mesh, idOrKey) {
    if (!mesh.client)
        return null;
    ...
    const matches = await mesh.client.getWhere(`metadata LIKE '%"key":"${escapedKey}"%' AND superseded_at IS NULL`, { limit: 50 });
    if (matches.length === 0)
        return null;
 ...
 *                query is given.
 */
export async function prime(mesh, query, opts = {}) {
    await mesh.init();
    if (!mesh.client) {
        throw new Error("Database client not initialized");
    }
