 ...
 * byte-identical.
 */
export async function exportJsonl(mesh, filePath) {
    await mesh.init();
    if (!mesh.client) {
 ...
 */
export async function importJsonl(mesh, source) {
    await mesh.init();
    if (!mesh.client) {
        throw new Error("Database client not initialized");
    }
    ...
    for (const row of rows.slice(1)) {
        if (row.table === "memory_entries") {
            ...
            if (!mesh.skillTable || skillIds.has(row.id)) {
                bump(row.table, false);
                continue;
            }
            ...
            if (!mesh.decisionEdgeTable || dedgeIds.has(row.id)) {
                bump(row.table, false);
                continue;
            }
            ...
            if (!mesh.graphTable || gedgeIds.has(row.id)) {
                bump(row.table, false);
                continue;
            }
            ...
            if (!mesh.revisionTable || revIds.has(row.id)) {
                bump(row.table, false);
                continue;
            }
 ...
 */
export async function staleMemoriesReport(mesh, opts = {}) {
    await mesh.init();
    if (!mesh.client) {
        throw new Error("Database client not initialized");
    }
 ...
 */
export async function doctor(mesh, opts = {}) {
    await mesh.init();
    const checks = [];
    // 1. Database reachable and populated
    try {
        const tables = mesh.client?.db ? await mesh.client.db.tableNames() : [];
    ...
    try {
        const drifted = await mesh.client.getWhere(`superseded_at IS NOT NULL AND (state IS NULL OR state != 'superseded')`, { limit: 1000 });
        checks.push({
