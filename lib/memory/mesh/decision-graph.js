 ...
 */
export async function orphanEdges(mesh, opts = {}) {
    await mesh.init();
 ...
 */
export function _coerceIdList(_mesh, value) {
 ...
 */
export function _isDecisionWrite(mesh, metadata, supersededIds) {
    if (!metadata)
 ...
 */
export async function _writeDecisionEdges(mesh, sourceId, metadata, supersededIds) {
    if (!mesh.decisionEdgeTable)
        return;
    ...
    for (const t of mesh._coerceIdList(metadata?.depends_on))
        addEdge(t, "depends-on");
    for (const t of mesh._coerceIdList(metadata?.justified_by))
        addEdge(t, "justified-by");
    for (const t of mesh._coerceIdList(metadata?.contradicts))
 ...
 */
export async function decisionLineage(mesh, memoryId, opts = {}) {
    await mesh.init();
    if (!mesh.decisionEdgeTable)
        return [];
 ...
 */
export async function _applyContradictionPenalty(mesh, results) {
 ...
 * it, with hop counts.
 */
export async function staleBeliefs(mesh, opts = {}) {
    await mesh.init();
    if (!mesh.client) {
    ...
    if (opts.memoryId) {
        ...
        const candidates = await mesh.client.getWhere(`metadata LIKE '%"status":"refuted"%'`, { limit: METADATA_SCAN_CAP });
 ...
 */
export async function recordOutcome(mesh, decisionId, outcome) {
    await mesh.init();
    if (!mesh.client) {
        throw new Error("Database client not initialized");
    }
