 ...
 */
export async function add(mesh: MemoryMesh, content: string, metadata: Record<string, any> = {}) {
    await mesh.init();
        ...
        if (mesh.client) {
            ...
            if (sanitizedMetadata.key) {
                try {
                    ...
                    const activeRecords = await mesh.client.getWhere(
 ...
 */
export async function addDocument(mesh: MemoryMesh,
    content: string,
    ...
    for (let i = 0; i < spans.length; i++) {
        ...
        if (lateVectors) {
            ...
            if (!mesh.client) throw new Error('Database client not initialized');
            const result = await mesh.client.add(record);
 ...
 */
export async function insertHeritage(mesh: MemoryMesh, memoryId: string, heritage: { intentChain: string[]; hypotheses: string[]; rationales: string[] }): Promise<void> {
    await mesh.init();
    if (!mesh.client) throw new Error("Database client not initialized");
    try {
 ...
 */
export async function _judgeMemoryWrite(mesh: MemoryMesh,
    newContent: string,
 ...
 */
export async function _emitAgenticDecisionBlock(mesh: MemoryMesh,
    judgment: { decision: string; mergedContent?: string; rationale?: string },
    neighborId: string,
    newContent: string,
) {
    if (!mesh.yamoTable) return;
    const ts = new Date().toISOString();
    const yamoText = [
        `agent: MemoryMesh_${mesh.agentId};`,
