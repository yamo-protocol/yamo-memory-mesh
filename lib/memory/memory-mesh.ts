...
import { getEmbeddingDimension, createSynthesizedSkillSchema, MEMORY_STATES, MemoryState, INDEX_CONFIG, } from "./schema.js";
...
export class MemoryMesh {
    ...
    async addDocument(
        ...
        let lateVectors: number[][] | null = null;
    ...
    _splitParagraphSpans(content: string, minChars: number, maxChars: number): Array<{ start: number; end: number }> {
        ...
        if (chunkEnd > chunkStart) spans.push({ start: chunkStart, end: chunkEnd });
        return spans;
    }
    ...
    async raptor(options: {
        ...
        perLevel: Array<{ level: number; clusters: number; summaries: number }>;
        treeRootId?: string;
    }> {
        await this.init();
        const out = { levelsBuilt: 0, summariesCreated: 0, perLevel: [] as Array<{ level: number; clusters: number; summaries: number }>, treeRootId: undefined as string | undefined };
        if (!this.enableLLM || !this.llmClient) {
            if (process.env.YAMO_DEBUG === 'true') {
        ...
        const toArray = (v: any): number[] | null => {
            if (!v || typeof v.length !== 'number' || v.length === 0) return null;
            return Array.isArray(v) ? v : Array.from(v as Iterable<number>);
        };
        let currentLayer: Array<{ id: string; content: string; vector: number[] }> = [];
        if (topic) {
        ...
        if (currentLayer.length === 0) return out;
        ...
        for (let level = 1; level <= maxLevels; level++) {
            ...
            const validSummaries = summaries.filter((s): s is { id: string; content: string; vector: number[] } => s !== null);
            ...
            out.levelsBuilt++;
            if (validSummaries.length === 0) break;
        ...
        for (let iter = 0; iter < maxIters; iter++) {
            ...
            for (let c = 0; c < k; c++) {
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    for (let d = 0; d < dim; d++) sum[d] += items[i].vector[d];
                    count++;
                }
                if (count === 0) continue;
    ...
    async _summarizeCluster(
        ...
        if (!this.enableLLM || !this.llmClient) return null;
...
[MEMORY CONTEXT]`;
    ...
    _recordRevision(memoryId: string, changes: Array<{ field: string; oldValue: unknown; newValue: unknown }>, actor?: string): void {
        if (!this.revisionTable || changes.length === 0) return;
        ...
        this.revisionTable.add(rows).catch((e: unknown) => {
            if (process.env.YAMO_DEBUG === "true") {
    ...
    async _resolveIdOrKey(idOrKey: string): Promise<MemoryRecord | null> {
        ...
        const matches = await this.client.getWhere(
            `metadata LIKE '%"key":"${escapedKey}"%' AND superseded_at IS NULL`,
            { limit: 50 },
        );
    ...
    async importJsonl(source: string | { text: string }): Promise<Record<string, { imported: number; skipped: number }>> {
        ...
        for (const row of rows.slice(1)) {
            if (row.table === "memory_entries") {
                ...
                if (!this.skillTable || skillIds.has(row.id)) { bump(row.table, false); continue; }
                ...
                if (!this.decisionEdgeTable || dedgeIds.has(row.id)) { bump(row.table, false); continue; }
                ...
                if (!this.graphTable || gedgeIds.has(row.id)) { bump(row.table, false); continue; }
                ...
                if (!this.revisionTable || revIds.has(row.id)) { bump(row.table, false); continue; }
    ...
    async doctor(opts: { indexThreshold?: number } = {}): Promise<{ ok: boolean; checks: Array<{ name: string; ok: boolean; detail: string }> }> {
        ...
        try {
            const tables: string[] = this.client?.db ? await this.client.db.tableNames() : [];
        ...
        try {
            const drifted = await this.client.getWhere(
