 ...
 */
export function recallAtK(results, gold, k) {
    if (gold.size === 0)
        return 0;
 ...
 */
export function reciprocalRank(results, gold) {
    for (let i = 0; i < results.length; i++) {
        if (gold.has(results[i].id))
            return 1 / (i + 1);
    }
    return 0;
}
 ...
 */
export function ndcgAtK(results, gold, k) {
    if (gold.size === 0)
        return 0;
    const topK = results.slice(0, k);
    let dcg = 0;
...
export function evaluateQuery(results, gold) {
    return {
